...
class GAug(object):
    def __init__(self, adj_matrix, features, labels, tvt_nids, cuda=-1, hidden_size=128, emb_size=32, n_layers=1, epochs=200, seed=-1, lr=1e-2, weight_decay=5e-4, dropout=0.5, gae=False, beta=0.5, temperature=0.2, name='debug', warmup=3, gnnlayer_type='gcn', jknet=False, alpha=1, sample_type='add_sample', feat_norm='row'):
        self.lr = lr
...
class GAug_model(nn.Module):
    ...
    def forward(self, adj, adj_orig, features):
        ...
        # print(len(np.nonzero(adj_orig)))
        # print(len(np.nonzero(adj_logits)))
        # print(len(np.nonzero(adj_new_normed)))
        nc_logits = self.nc_net(adj_orig, features)
        return nc_logits, adj_orig
