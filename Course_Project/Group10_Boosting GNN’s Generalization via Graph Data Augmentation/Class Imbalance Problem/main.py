...
def train(epoch):
    ...
    if args.setting == 'recon_newG' or args.setting == 'recon' or args.setting == 'newG_cls':
        ...
        embed, labels_new, idx_train_new, adj_up = utils.recon_upsample(embed, labels, idx_train, adj=adj.detach().cpu().to_dense(),portion=args.up_scale, im_class_num=im_class_num)
        ...
        if torch.cuda.is_available():
            adj_new = torch.mul(adj_up.cuda(), adj_new.cuda())
        else:
            adj_new = torch.mul(adj_up.cpu(), adj_new.cpu())
